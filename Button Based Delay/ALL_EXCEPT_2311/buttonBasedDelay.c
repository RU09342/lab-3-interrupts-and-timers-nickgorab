...
void main(void) {
  ...
  TA0CCR0 = 200;                     // 10 Hz
  TA0CTL = TASSEL_1 + MC_1 + ID_3;   //SMCLK, Up-Mode, Predivider of 8
...
__interrupt void Port_1(void) {
    ...
    TA0CCR0 = 0;                    // Resets the CCR0 value

    while((BUTTON1 & B1_IN) == 0){  // Sees if the button is pressed
        TA0CCR0 = TA0CCR0 + 1;      // Increases the CCR0 value depending on how long button is pressed
    }
...
__interrupt void Port2(void) {
    ...
    TA0CCR0 = 200;         // Resets the CCR0 value to 10 Hz
